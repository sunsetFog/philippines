          ...
          <div class="query">
              <el-row>
                <el-form :inline="true" label-width="80px">
                  ...
                  <el-col :span="3">
                    <el-button type="primary" icon="el-icon-search" @click="query" v-if="gamesetqznnlist">查询</el-button>
                  </el-col>
                </el-form>
              </el-row>
            </div>
...
function getlist (that) {
  request({
    url: that.public.url + '/gameset/qznnlist',
    method: 'post',
    data: {
      game_id: 1101,
      type: 0,
...
function getRate (that) {
    request({
      ...
      for(let i=0;i<res.data.length;i++){
        if(res.data[i].id==1101&&res.data[i].type==0){
