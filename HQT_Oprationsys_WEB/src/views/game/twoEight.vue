<template>
  <div class="app-container brclassic">
       <el-tabs type='card' @tab-click="cardTab">
        <el-tab-pane label="基本设置">
          <el-form :model="form" :rules="rules" ref="form">
            ...
            <el-form-item label="抢庄时长：" :label-width="formLabelWidth" prop="masterdur" >
              <el-input v-model="form.masterdur" type="number" style="width:300px"></el-input>
              <span>秒</span>
            </el-form-item>
            <el-form-item label="投注时长：" :label-width="formLabelWidth" prop="countdown">
              <el-input v-model="form.countdown" type="number" style="width:300px"></el-input>
               <span>秒</span>
            </el-form-item>
            <el-form-item label="准备时长：" :label-width="formLabelWidth" prop="ready_dur">
              <el-input v-model="form.ready_dur" type="number" style="width:300px"></el-input>
               <span>秒</span>
            </el-form-item>
          ...
          <div class="query">
              <el-row>
                <el-form :inline="true" label-width="80px">
                  ...
                  <el-col :span="3">
                    <el-button type="primary" icon="el-icon-search" @click="query" v-if="gamesetebglist">查询</el-button>
                  </el-col>
                </el-form>
              </el-row>
            </div>
            ...
            width='170'
              label="场地操作">
              <template slot-scope="scope">
                <el-button @click="edit(scope.row)" type="text" size="small" v-if='gamesetqznngameset'>修改</el-button>
                <el-button @click="up(scope.row)" type="text" size="small" class='green' v-if="scope.row.status === '0' && gamesetupdatetablestatus">开启</el-button>
                <el-button @click="down(scope.row)" type="text" size="small" class="red" v-if="scope.row.status === '1' && gamesetupdatetablestatus">禁用</el-button>
  ...
  <div class="el-table__body-wrapper is-scrolling-none">
      ...
      <tbody>
        <tr class="el-table__row">
            ...
            <td crowspan="1" colspan="1" style="width:30%;">
              <div class="cell">
                <el-button type="primary" v-if="rate_active && gamenamesetroyatly" @click="sureRate">保存</el-button>
    ...
    <el-form :model="form1" :rules="rules1" ref="form1">
       <el-form-item label="场次名称:" :label-width="formLabelWidth" disable>
        <el-input v-model="form1.name" clearable style="width:80%" disabled="disabled"></el-input>
      </el-form-item>
        <el-form-item label="底分:" :label-width="formLabelWidth"  prop="base_score">
        <el-input v-model="form1.base_score" clearable style="width:80%"></el-input>
      </el-form-item>
       <el-form-item label="入场限制:" :label-width="formLabelWidth"  prop="ent_limit">
        <el-input v-model="form1.ent_limit" clearable style="width:80%"></el-input>
      </el-form-item>
       <el-form-item label="投注最高倍数:" :label-width="formLabelWidth" prop="bet_max_multiple">
        <el-input v-model="form1.bet_max_multiple" clearable style="width:80%"></el-input>
      </el-form-item>
      <!-- <el-form-item label="排序:" :label-width="formLabelWidth" prop='sort'>
        <el-input v-model="form1.sort" clearable style="width:80%" typr='number'></el-input>
      </el-form-item> -->
...
export default {
  data() {
   ...
   var base_score =(rule, value, callback)=>{
     if(value*1<=0 || isNaN(value) ) {
          callback(new Error('请输入正数'));
        } else {
          callback();
   ...
   var ent_limit =(rule, value, callback)=>{
     if(value*1<=0 || isNaN(value) ) {
          callback(new Error('请输入正数'));
        } else {
          callback();
   ...
   var bet_max_multiple =(rule, value, callback)=>{
     if(value*1<=0 || isNaN(value) ) {
          callback(new Error('请输入正数'));
        } else {
          callback();
   ...
   var ent_limit2 = (rule, value, callback)=>{ 
        if(value*1<=0 || value.indexOf('.')>0 || isNaN(value)) {
          callback(new Error('请输入正整数'));
        } else {
          callback();
    ...
    return {
      ...
      rules: {
        ...
        countdown:[{required: true, validator: ent_limit2, trigger: 'blur'}],
        masterdur:[{required: true, validator: ent_limit2, trigger: 'blur'}],
        ready_dur:[{required: true, validator: ent_limit2, trigger: 'blur'}]
      },
      rules1: {
        table: [
        ...
        sort: [{required: true, validator: solts, trigger: 'blur'}],
         base_score: [{required: true, validator: base_score, trigger: 'blur'}],
         ent_limit: [{required: true, validator: ent_limit, trigger: 'blur'}],
         bet_max_multiple: [{required: true, validator: bet_max_multiple, trigger: 'blur'}],
      },
      statuslist: [
        {name:'启用',id: '1' },
        {name:'禁用',id: '0' }
  ...
  methods: {
    ...
    modifyRate(){
      ...
      this.rate_input = (chumul(this.pumping_rate*1,100)).toFixed(4);
    },
    sureRate(){
      let that = this;
    ...
    sure () {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
            ...
            data: {
                  game_id: 1201,
                  bet_count_down: this.form.countdown,
                  grab_master_dur:this.form.masterdur,
                  depict: this.form.depict,
                  status: this.form.status,
                  ready_dur:this.form.ready_dur,
                  type:0,
            }
          }).then(res => {
            that.$message({
    ...
    edit (row) {
      ...
      this.form1.base_score = row.base_score
      this.form1.ent_limit = row.ent_limit
      this.form1.bet_max_multiple = row.bet_max_multiple
      if (row.status == '1') {
        this.form1.status = '1'
      } else {
        this.form1.status = '0'
    ...
    sure1 () {
        ...
        if (valid) {
          ...
          if (this.title === '修改桌子') {
             request({
              url: that.public.url + '/gameset/qznngameset',
              method: 'post',
              data: {
                    id: this.id,
                  // sortid:this.form1.sort,
                    name: this.form1.name,
                    base_score: this.form1.base_score,
                    ent_limit: this.form1.ent_limit,
                    bet_max_multiple: this.form1.bet_max_multiple,
...
function getlist (that) {
  request({
    url: that.public.url + '/gameset/ebglist',
    method: 'post',
    data: {
      game_id: 1201,
      type: 0,
...
function getinfo (that) {
  request({
      ...
      that.form.masterdur = res.data.grab_master_dur
      that.form.countdown = res.data.bet_count_down
      that.form.ready_dur = res.data.ready_dur
...
function getRate (that) {
    request({
      ...
      for(let i=0;i<res.data.length;i++){
        if(res.data[i].id==1201&&res.data[i].type==0){
