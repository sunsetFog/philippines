<template>
  <div class="batchimport proxyaccount">
    <el-tabs v-model="activeName2" type="border-card" @tab-click="handleClick">
      <div class="batchdown">
        <el-tab-pane label="批量导入" name="first" style="height:auto;padding-bottom: 53px;" v-if="paychgmoneybatchagent">
...
<div class="pagingbox" v-if="showtable">
    <div class="paging" style="margin-top: 20px;">
      ...
      <el-tab-pane label="导入历史" name="second" v-if="paychgmoneygetagentup">
        <div class="batchdown">
...
import { getToken } from '../../utils/auth';
  export default {
    ...
    computed: {
      ...
      action () {
        return this.public.url + '/paychgmoney/batchagent'
      },
      datatype () {
        return {
      ...
      ...mapGetters([
        'paychgmoneygetagentup',
        'paychgmoneybatchagent'
      ])
    },
    filters: {
      type(val) {
    ...
    excel () {
      window.open('../../../static/批量代理.xls')
    },
    excelup (row) {
      let that = this
      request({
        url: that.public.url + '/backend/paychgmoney/dwnagentupfile',
        method: 'post',
        data: {
          mid: row.id
        }
    ...
    allpass () {
      let that = this
      request({
        url: that.public.url + '/backend/paychgmoney/submitagent',
        method: 'post',
        data: {
        }
      }).then(res => {
      ...
      checkdel () {
        ...
        let id = this.multipleSelection.map(val => {
          ...
          this.$confirm('此操作将删除这些申请, 是否继续?', '提示', {
            ...
            request({
                url: that.public.url + '/backend/paychgmoney/delagent',
                method: 'post',
                data: {
                  ids: ids,
                  tbtype: tbtypes
      ...
      del (row) {
        ...
        this.$confirm('此操作将删除该数据, 是否继续?', '提示', {
          ...
          request({
              url: that.public.url + '/backend/paychgmoney/delagentup',
              method: 'post',
              data: {
                mid: row.id
              }
  ...
  function getlist (that) {
    request({
      url: that.public.url + '/backend/paychgmoney/getagentlist',
      method: 'post',
      data: {
...
function getaccount (that) {
  ...
  function getchargelogs (that) {
    ...
    request({
      url: that.public.url + '/backend/paychgmoney/getagentup',
      method: 'post',
      data: {
        date_from: start,
        date_to: end,
