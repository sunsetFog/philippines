<template>
  <div class="app-container">
    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column label="玩家输额">
        <template slot-scope="scope">{{scope.row.update_time | time}}</template>
      </el-table-column>
      <el-table-column label="平台税收">
        <template slot-scope="scope">{{scope.row.update_time | time}}</template>
      </el-table-column>
      <el-table-column label="可用金额">
        <template slot-scope="scope">{{scope.row.update_time | time}}</template>
      </el-table-column>
      <el-table-column label="玩家Ip">
        <template slot-scope="scope">{{scope.row.update_time | time}}</template>
      </el-table-column>
    </el-table>
    <div class="pagingbox">
      ...
      <el-form :model="form" :rules="rules" ref="form">
        ...
        <el-form-item label="域名" :label-width="formLabelWidth" prop="domain">
          <el-select v-model="form.domain" multiple filterable>
            <el-option v-for="item in domainlist" :key="item.id" :label="item.url" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: "请输入玩家渠道", trigger: "blur" }],
  ...
  computed: {
    ...mapGetters(["orgadd", "orgdel", "orgupdate", "orggetlist", "org"])
  },
  methods: {
    reset(form) {
